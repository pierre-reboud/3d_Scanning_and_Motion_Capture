...
public:
	...
	{
		Eigen::Vector3d position_vec = _x - this->m_center;
		return position_vec.dot(position_vec) - std::pow(this->m_radius,2);
...
public:
	...
	{
		Eigen::Vector3d position_vec = _x - this->m_center;
		double first_term = position_vec.dot(position_vec) + std::pow(this->m_radius, 2) - std::pow(this->m_a, 2);
		double second_term = 4.0d*std::pow(this->m_radius,2);
		double third_term = position_vec.segment(0,2).dot(position_vec.segment(0,2));
		return std::pow(first_term,2) - second_term * third_term;
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		for(int i = 0; i < m_numCenters; i++){
			result += this->m_coefficents[i] * EvalBasis((m_funcSamp.m_pos[i]-_x).norm()); 
		}
		return result + this->m_coefficents.segment(m_numCenters, 3).dot(_x) + this->m_coefficents[m_numCenters + 3];
...
#define phi(i,j) EvalBasis((m_funcSamp.m_pos[i]-m_funcSamp.m_pos[j]).norm())
	...
	void BuildSystem()
	{
		...
		for(int i = 0; i<m_numCenters; i++){
			for(int j = i; j<m_numCenters; j++){
				A(i,j) = phi(i,j);
				A(i + m_numCenters,j) = phi(i + m_numCenters,j);
				if (j!=i){
					A(j,i) = A(i,j);
					A(j + m_numCenters,i) = phi(j + m_numCenters,i);
				}
			}
			A.block<1,3>(i,m_numCenters) = m_funcSamp.m_pos[i];
			A.block<1,3>(i + m_numCenters, m_numCenters) = m_funcSamp.m_pos[i + m_numCenters];
			b(i) = m_funcSamp.m_val[i];
			b(i + m_numCenters) = m_funcSamp.m_val[i + m_numCenters];
		}
		A.col(m_numCenters + 3) = Eigen::MatrixXd::Constant(2 * m_numCenters, 1, 1.0d);
